...
CHANGE: Data set is shared by [Kuka et al, 2020](https://doi.org/10.1257/pol.20180352). Here we showcase the functionality of the package by using a subset, which is available in the package and can be loaded by

```{r data, eval = TRUE}
  library(didnp)
  library(ggplot2)
  data(unemp, package = "didnp")
  head(unemp)
```
...
Because there is a gap in the 'year', we create artificial 'period' variable that does not have a gap

```{r sub, eval=TRUE}
year_min <- min(unemp$year)
unemp$period <- ifelse(
  unemp$year < 2012,
  unemp$year - year_min + 1,
  unemp$year - year_min)
table(unemp$period)
table(unemp$period, unemp$year)
table(unemp$period, unemp$treatment_period)
```
...
```{r sub, eval=TRUE}
# get the subsample
unemp$mysmpl <- unemp$age >= 25
table(unemp$mysmpl)

# describe 'treatment_period'
table(unemp$treatment_period)
```

> Define the formula that we will use:

```{r formula, eval = TRUE}
form1 <- y ~ unemp + sex + statefip + school + age + yrimmig + educ | period | eligible | treatment_period
```
...
COMMENT: We do find evidence against the null hypothesis that the bias stability condition holds.
...
table(unemp$educ)
# Create 3 categories:
ifelse(unemp$educ <= 3, "2-3 years", ifelse(unemp$educ <= 7, "6-7 years", "8-11 years")) -> unemp$edu
factor(unemp[tym1b$esample, "edu"]) -> edu
table(edu)
unemp[tym1b$esample, "sex"] -> sex
as.numeric(unemp[tym1b$esample, "age"]) -> age
```

Here `unemp` is continuous.

First, use one categorical `by` variable:

```{r gr_, eval=TRUE}
tym1b_gr_edu <- didnpplot(
  obj = tym1b,
  level = 95,
  by = edu[tym1b$sample1],
  xlab = "Education",
  ylab = "ATET"
  )
...
tym1b_gr_sex <- didnpplot(
  ...
  xlab = "Sex",
  ylab = "ATET",
  by.labels.values = data.frame(c(1,2), c("Female", "Male"))
)
...
```{r gr_age, eval=TRUE}
tym1b_gr_unemp <- didnpplot(
  obj = tym1b,
  level = 95,
  by = unemp$unemp[tym1b$esample][tym1b$sample1],
  n.intervals = 10,
  xlab = "Unemployment"
)

tym1b_gr_unemp$data.a
tym1b_gr_unemp$data.b

tym1b_gr_unemp$plot.a
tym1b_gr_unemp$plot.b
```
